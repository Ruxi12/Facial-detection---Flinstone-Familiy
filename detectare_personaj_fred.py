# -*- coding: utf-8 -*-
"""Detectare_personaj.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1_zA8y3BIcwBjVFHpIzlkzYoVSMEnzeA2
"""
...
all_negative_samples = all_negative_samples[:12000]
all_negative_samples = np.array(all_negative_samples) / 255
print(f"fred = {fred.shape}")
# MODEL PENTRU FRED
negative_fred = np.concatenate((barney, betty, wilma, unknown, all_negative_samples), axis=0)
flip_fred = np.array([np.fliplr(face) for face in fred])
positive_fred = np.concatenate((fred, flip_fred), axis=0)
# positive_fred = np.array(fred)
# Convert lists to numpy arrays for model processing
negative_fred = np.array(negative_fred)
positive_fred = np.array(positive_fred)

print(f"negative_fred = {negative_fred.shape}")
print(f"positive_fred = {positive_fred.shape}")

fred = []
flip_fred = []

labels_positive_fred = np.ones(len(positive_fred))
labels_negative_fred = np.zeros(len(negative_fred))
print(labels_positive_fred.shape, labels_negative_fred.shape)

# Combine data and labels correctly
data_fred = np.concatenate((positive_fred, negative_fred), axis=0)
labels_fred = np.concatenate((labels_positive_fred, labels_negative_fred), axis=0)
print(labels_fred.shape)

from sklearn.utils import shuffle

# Now shuffle the data and labels
data_fred, labels_fred = shuffle(data_fred, labels_fred, random_state=0)

print(data_fred.shape, labels_fred.shape)


from sklearn.model_selection import train_test_split
# positive_fred = None
# negative_fred = None
# labels_positive_fred = None
# labels_negative_fred = None

# x_train_fred, x_val_fred, y_train_fred, y_val_fred = train_test_split(data_fred, labels_fred, test_size=0.2, random_state=0)

# print(barney.shape)
print(fred.shape)
print(flip_fred.shape)
# print(betty.shape)
# print(unknown.shape)
...
non_fred_data_val = np.concatenate((val_barney, val_betty, val_wilma, val_unknown), axis=0)
validation_data_fred = np.concatenate((val_fred, non_fred_data_val), axis=0)

# Create labels for validation (1 for Fred, 0 for Non-Fred)
val_labels_fred_positive = np.ones(len(val_fred))
val_labels_fred_negative = np.zeros(len(non_fred_data_val))
val_labels_fred = np.concatenate((val_labels_fred_positive, val_labels_fred_negative), axis=0)

# Shuffle the validation data and labels together
validation_data_fred, val_labels_fred = shuffle(validation_data_fred, val_labels_fred, random_state=0)

model_fred = FaceClassifier()

# Compile the model
model_fred.compile(optimizer='adam',
...
# Train the model
history = model_fred.fit(
    data_fred, labels_fred,
    batch_size=batch_size,
    epochs=epochs,
    validation_data=(validation_data_fred, val_labels_fred),
    callbacks=[early_stopping]
)

# Predict on validation data
val_predictions = model_fred.predict(validation_data_fred)

# Evaluate the model
loss, accuracy = model_fred.evaluate(validation_data_fred, val_labels_fred)
...
plt.savefig('training_validation_loss_fred.png')
plt.show()

model_fred.save_weights('fred_face_classifier_weights4.h5')
