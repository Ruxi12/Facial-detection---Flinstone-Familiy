...
# all_negative_samples = all_negative_samples[:500]
# all_negative_samples = np.array(all_negative_samples) / 255
print(f"fred = {fred.shape}")

# MODEL PENTRU WILMA
negative_wilma = np.concatenate((fred, betty, barney, unknown), axis=0)
flip_wilma = np.array([np.fliplr(face) for face in wilma])
positive_wilma = np.concatenate((wilma, flip_wilma), axis=0)
# positive_fred = np.array(fred)
# Convert lists to numpy arrays for model processing
negative_wilma = np.array(negative_wilma)
positive_wilma = np.array(positive_wilma)

print(f"negative_wilma = {negative_wilma.shape}")
print(f"positive_wilma = {positive_wilma.shape}")

labels_positive_wilma = np.ones(len(positive_wilma))
labels_negative_wilma = np.zeros(len(negative_wilma))
print(labels_positive_wilma.shape, labels_negative_wilma.shape)

# Combine data and labels correctly
data_wilma = np.concatenate((positive_wilma, negative_wilma), axis=0)
labels_wilma = np.concatenate((labels_positive_wilma, labels_negative_wilma), axis=0)
print(labels_wilma.shape)

from sklearn.utils import shuffle

# Now shuffle the data and labels
data_wilma, labels_wilma = shuffle(data_wilma, labels_wilma, random_state=0)

print(data_wilma.shape, labels_wilma.shape)
...
non_wilma_data_val = np.concatenate((val_fred, val_betty, val_barney, val_unknown), axis=0)
validation_data_wilma = np.concatenate((val_wilma, non_wilma_data_val), axis=0)

# Create labels for validation (1 for Fred, 0 for Non-Fred)
val_labels_wilma_positive = np.ones(len(val_wilma))
val_labels_wilma_negative = np.zeros(len(non_wilma_data_val))
val_labels_wilma = np.concatenate((val_labels_wilma_positive, val_labels_wilma_negative), axis=0)

# Shuffle the validation data and labels together
validation_data_wilma, val_labels_wilma = shuffle(validation_data_wilma, val_labels_wilma, random_state=0)

model_wilma = FaceClassifier()

# Compile the model
model_wilma.compile(optimizer='adam',
...
# Train the model
history = model_wilma.fit(
    data_wilma, labels_wilma,
    batch_size=batch_size,
    epochs=epochs,
    validation_data=(validation_data_wilma, val_labels_wilma),
    callbacks=[early_stopping]
)

# Predict on validation data
val_predictions = model_wilma.predict(validation_data_wilma)

# Evaluate the model
loss, accuracy = model_wilma.evaluate(validation_data_wilma, val_labels_wilma)

# Print the results
print(f"Loss on validation set: {loss}")
print(f"Accuracy on validation set: {accuracy}")

model_wilma.save_weights('wilma_face_classifier_weights5.h5')
